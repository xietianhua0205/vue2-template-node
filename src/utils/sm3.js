 ...
 * @returns {string}
 */
function signature (queryParam, formData, json, timestamp, nonceStr) {
  let objParam = Object.assign({}, queryParam, formData)
  let param = ''
  if (Object.keys(objParam).length > 0) {
    let newObjParam = {}
    Object.keys(objParam).sort().forEach(function (key) {
      if (objParam[key] !== undefined && objParam[key] !== null) {
        newObjParam[key] = objParam[key]
      }
    })
    for (let userKey in newObjParam) {
      let v = newObjParam[userKey]
      if (typeof newObjParam[userKey] == 'object') {
        v = JSON.stringify(v)
      }
      if (param) {
        param = param + '&' + userKey + '=' + v
        continue
      }
      param = userKey + '=' + v
    }
  ...
  if (json) {
    if (typeof json === 'string') {
      param += json
    } else if (Object.keys(json).length > 0 || json instanceof Array) {
      param += JSON.stringify(json)
    }
  }
  timestamp = timestamp || Date.now()
  nonceStr = nonceStr || getRandomString(8)
  if (param) {
    return sm3(param + '' + '&timestamp=' + timestamp + '&nonceStr=' + nonceStr)
  }
  return sm3('timestamp=' + timestamp + '&nonceStr=' + nonceStr)
}
...
export { sm3, signature, getRandomString }
