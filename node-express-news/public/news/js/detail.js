...
$(function() {
  ...
  $('.comment_list_con').delegate('a,input', 'click', function() {
    ...
    if (sHandler.indexOf('reply_sub') >= 0) {
      ...
      //           <div class="user_name2">xiaole 回复 用户李思</div>
      //           <div class="reply_text">${ comment }</div>
      //       </div>
      ...
      $.ajax({
          ...
          success: function (resp) {
              if (resp.errno == "0") {
                  ...
                  var comment_html = ""
                if(parent_id){
                 let current_html = `<div className="reply_text_con fl">
                    <div className="user_name2">${comment.parent.user.nick_name}</div>
                    <div className="reply_text">${comment}</div>
                  </div>`
                  console.log($(this).find('.comment_text'));
                 debugger
                  $this.before(current_html)
                  // 请空输入框
                  $this.prev().val('')
                  // 关闭
                  $this.parent().hide()
